...
static void PrintAbortPointerArrayCandidate(const char* name, uint64_t addr) {
	...
	for (int i = 0; i < 8; i++) {
		...
		const auto child_name = std::string(name) + "[" + std::to_string(i) + "]";
...
const auto stack_name = std::string("stack[") + std::to_string(i) + "]";
				PrintAbortPointerCandidate(stack_name.c_str(), value);
				PrintAbortPointerArrayCandidate(stack_name.c_str(), value);
		}
	}
...
static KYTY_SYSV_ABI int MtxInit(LibKernel::PthreadMutex* mtx, int type) {
	...
	if ((type & mtx_recursive) == 0) {
                return mtx_result(LibKernel::PthreadMutexInit(mtx, static_cast<const LibKernel::PthreadMutexattr*>(nullptr)));
        }
