...
Network::Id Network::HttpCreateConnection(Id tmpl_id, const char* server_name, const char* scheme,
        ...
        if (server_name != nullptr && scheme != nullptr && HttpValidTemplate(tmpl_id)) {
                ...
                if (port != 0) {
                        cn.url += ":" + std::to_string(static_cast<unsigned>(port));
                }
...
#else
        // Round 28: POSIX hosts speak Linux errno numbers, the guest speaks
        // FreeBSD numbers (POSIX_* constants). Translate the common socket
        // errnos using the HOST macros so the table stays correct on any
        // POSIX platform; unknown values fall back to EIO (fail-closed).
        switch (error) {
                ...
                case EAGAIN:
#endif
                        posix_error = Posix::POSIX_EAGAIN; break;
...
#if !defined(_WIN32)
// Round 28: guest (FreeBSD/PS4) SOL_SOCKET option numbers vs Linux. Most low
// values coincide; SO_REUSEPORT (guest 0x200) and a few PS4-specific values
// (SO_NBIO 0x1200 handled separately, SO_USELOOPBACK, SO_SHARECKET) differ.
static int TranslateSocketOptionName(int host_level, int optname) {
        if (host_level == SOL_SOCKET) {
                switch (optname) {
                        case 0x0004: return SO_REUSEADDR;      // 4 on both
                        case 0x0008: return SO_KEEPALIVE;      // 8 on FreeBSD, 9 on Linux
                        case 0x0010: return SO_DONTROUTE;      // 16 on FreeBSD, 5 on Linux
                        case 0x0020: return SO_BROADCAST;      // 0x20 on both
                        case 0x0080: return SO_LINGER;         // 0x80 on both
                        case 0x0100: return SO_OOBINLINE;      // 0x100 on both
                        case 0x0200: return SO_REUSEPORT;      // FreeBSD 0x200, Linux 0xf
                        case 0x1001: return SO_SNDBUF;         // 0x1001 on both
                        case 0x1002: return SO_RCVBUF;         // 0x1002 on both
                        ...
                        case 0x1200: return -1;                // SO_NBIO: handled by caller
                        default: return -1;                    // fail closed on unknown
                }
        }
        if (host_level == IPPROTO_TCP) {
                switch (optname) {
                        case 1: return TCP_NODELAY;            // 1 on both
                        default: return -1;
                }
        }
        return optname;   // other levels (IPPROTO_IP etc.): numbers coincide
}

// Round 28: host errno (Linux) -> guest POSIX_* (FreeBSD numbering), used for
// SO_ERROR reporting. Mirrors the SetPosixSocketError translation.
static int HostErrnoToPosix(int host_errno) {
        switch (host_errno) {
                ...
                case ECONNABORTED: return Posix::POSIX_ECONNABORTED;
                case ECONNREFUSED: return Posix::POSIX_ECONNREFUSED;
                case ECONNRESET: return Posix::POSIX_ECONNRESET;
                case EDESTADDRREQ: return Posix::POSIX_EDESTADDRREQ;
                case EFAULT: return Posix::POSIX_EFAULT;
                case EHOSTUNREACH: return Posix::POSIX_EHOSTUNREACH;
                case EINPROGRESS: return Posix::POSIX_EINPROGRESS;
                case EINTR: return Posix::POSIX_EINTR;
                case EINVAL: return Posix::POSIX_EINVAL;
                case EISCONN: return Posix::POSIX_EISCONN;
                case EMFILE: return Posix::POSIX_EMFILE;
                case EMSGSIZE: return Posix::POSIX_EMSGSIZE;
                case ENOBUFS: return Posix::POSIX_ENOBUFS;
                case ENETDOWN: return Posix::POSIX_ENETDOWN;
                case ENETRESET: return Posix::POSIX_ENETRESET;
                case ENETUNREACH: return Posix::POSIX_ENETUNREACH;
                case ENOMEM: return Posix::POSIX_ENOMEM;
                ...
                case ESHUTDOWN: return Posix::POSIX_ESHUTDOWN;
                case ETIMEDOUT: return Posix::POSIX_ETIMEDOUT;
                case EALREADY: return Posix::POSIX_EALREADY;
                default: return Posix::POSIX_EIO;
        }
}
#endif
...
static int ConvertMessageFlags(int flags) {
        ...
        return host_flags;
#else
        // Round 28: the guest speaks FreeBSD message-flag values. The four
        // low flags happen to match Linux, but MSG_NOSIGNAL differs (guest
        // 0x20000 vs Linux 0x4000), so the translation below is explicit.
        constexpr int guest_msg_peek      = 0x00000002;
        constexpr int guest_msg_dontroute = 0x00000004;
        constexpr int guest_msg_waitall   = 0x00000040;
        constexpr int guest_msg_dontwait  = 0x00000080;
...
// Round 28: guest (PS4/FreeBSD) sockaddr <-> host sockaddr conversion is
// platform-independent (sockaddr_in layout is identical), so both helpers
// are now compiled on every host instead of Win32 only.
...
// (the former #endif guarding these two helpers on Win32 was removed)
...
int KYTY_SYSV_ABI NetResolverStartNtoa(int rid, const char* hostname, void* addr, int timeout,
...
#else
        // Round 28: real POSIX resolution via getaddrinfo (identical flow).
        if (!EnsureSocketBackend()) {
                return NET_ERROR_ENETDOWN;
        }
        ...
        if (ret != 0 || result == nullptr) {
                ...
                return ret == EAI_NONAME ? NET_ERROR_RESOLVER_ENOHOST
                                         : NET_ERROR_RESOLVER_EINTERNAL;
        }
...
#else
        // Round 28: guest family value 28 is FreeBSD's AF_INET6; the host
        // macro differs on Linux (10). Convert explicitly.
        const int host_af = ConvertFamily(af);
        if (host_af < 0) {
                return nullptr;
        }
        if (::inet_ntop(host_af, const_cast<void*>(src), dst, size) == nullptr) {
                return nullptr;
        }
#endif
...
// Round 28: real MAC formatting for non-zero addresses (the zero address keeps
// the historical hardcoded path for back-compat with existing callers).
int KYTY_SYSV_ABI NetEtherNtostrReal(const NetEtherAddr* n, char* str, size_t len) {
        if (n == nullptr || str == nullptr || len < 18) {
                return NET_ERROR_EINVAL;
        }
        snprintf(str, len, "%02x:%02x:%02x:%02x:%02x:%02x", n->data[0], n->data[1], n->data[2],
                 n->data[3], n->data[4], n->data[5]);
        return OK;
}

// Round 28: parse a MAC string into NetEtherAddr (inverse of Ntostr).
int KYTY_SYSV_ABI NetEtherStrton(const char* str, NetEtherAddr* n) {
        if (str == nullptr || n == nullptr) {
                return NET_ERROR_EINVAL;
        }
        unsigned bytes[6] = {};
        if (sscanf(str, "%02x:%02x:%02x:%02x:%02x:%02x", &bytes[0], &bytes[1], &bytes[2],
                   &bytes[3], &bytes[4], &bytes[5]) != 6) {
                return NET_ERROR_EINVAL;
        }
        for (int i = 0; i < 6; ++i) {
                n->data[i] = static_cast<uint8_t>(bytes[i]);
        }
        return OK;
}
...
int KYTY_SYSV_ABI Socket(int family, int type, int protocol) {
        ...
        return fd;
#else
        // Round 28: real POSIX socket creation (AF_INET/AF_INET6 are native).
        const int type_clean = type & 0x7fffffff;
        const bool nonblock  = (type & 0x80000000) != 0;
        NativeSocket socket  = ::socket(host_family, type_clean, protocol);
        if (socket == INVALID_NATIVE_SOCKET) {
                return SetPosixSocketError();
        }
...
int KYTY_SYSV_ABI Bind(int s, const void* addr, uint32_t addrlen) {
        ...
        return 0;
#else
        // Round 28: real POSIX bind (same sockaddr conversion).
...
int KYTY_SYSV_ABI Accept(int s, void* addr, uint32_t* addrlen) {
        ...
        if (addr != nullptr || addrlen != nullptr) {
                ...
                if (ConvertHostSockaddr(&host_addr, static_cast<int>(host_addrlen), addr,
                                        addrlen) != 0) {
                        SocketClose(fd);
                        return -1;
                }
...
#else
        // Round 28: real POSIX getsockname.
        sockaddr_storage host_addr {};
        socklen_t        host_addrlen = sizeof(host_addr);
...
int KYTY_SYSV_ABI Getsockopt(int s, int level, int optname, void* optval, uint32_t* optlen) {
        ...
        *optlen = static_cast<uint32_t>(len);
        return 0;
#else
        // Round 28: real POSIX getsockopt with option-name translation
        // (guest FreeBSD values differ from Linux for a few SOL_SOCKET opts).
        const int host_level = ConvertSocketOptionLevel(level);
        int       host_opt   = TranslateSocketOptionName(host_level, optname);
        if (host_opt < 0) {
        ...
        if (::getsockopt(socket, host_level, host_opt, static_cast<char*>(optval), &len) != 0) {
                return SetPosixSocketError();
        }
        if (host_level == SOL_SOCKET && host_opt == SO_ERROR && len >= sizeof(int)) {
                // SO_ERROR carries a host errno: translate to guest numbering.
                *static_cast<int*>(optval) =
                    HostErrnoToPosix(*static_cast<const int*>(optval));
        }
        *optlen = static_cast<uint32_t>(len);
        return 0;
#endif
...
int KYTY_SYSV_ABI Setsockopt(int s, int level, int optname, const void* optval, uint32_t optlen) {
        ...
        return 0;
#else
        // Round 28: real POSIX setsockopt with option translation.
        constexpr int ORBIS_SO_NBIO = 0x1200;
        const int     host_level    = ConvertSocketOptionLevel(level);
        if (host_level == SOL_SOCKET && optname == ORBIS_SO_NBIO && optlen >= sizeof(int)) {
                const int flags = ::fcntl(socket, F_GETFL, 0);
                if (flags < 0) {
                        return SetPosixSocketError();
                }
                const int new_flags =
                    (*static_cast<const int*>(optval) != 0 ? flags | O_NONBLOCK : flags & ~O_NONBLOCK);
                if (::fcntl(socket, F_SETFL, new_flags) != 0) {
                        return SetPosixSocketError();
                }
                return 0;
        }

        const int host_opt = TranslateSocketOptionName(host_level, optname);
        if (host_opt < 0) {
                *Posix::GetErrorAddr() = Posix::POSIX_ENOPROTOOPT;
                return -1;
        }

        if (::setsockopt(socket, host_level, host_opt, static_cast<const char*>(optval),
...
int64_t KYTY_SYSV_ABI Sendto(int s, const void* buf, uint64_t len, int flags, const void* addr,
        ...
        return result;
#else
        // Round 28: real POSIX send/sendto (ssize_t -> int64_t).
        const int host_flags = ConvertMessageFlags(flags);
        if (host_flags < 0) {
                return -1;
        }
        ...
        ssize_t   result   = 0;
        if (addr == nullptr) {
                result = ::send(socket, static_cast<const void*>(buf),
                                static_cast<size_t>(host_len), host_flags);
        } else {
                sockaddr_storage host_addr {};
                int              host_addrlen = 0;
                if (ConvertGuestSockaddr(addr, addrlen, &host_addr, &host_addrlen) != 0) {
                ...
                result = ::sendto(socket, static_cast<const void*>(buf),
                                  static_cast<size_t>(host_len), host_flags,
...
int64_t KYTY_SYSV_ABI Recvfrom(int s, void* buf, uint64_t len, int flags, void* addr,
        ...
        return result;
#else
        // Round 28: real POSIX recv/recvfrom (ssize_t -> int64_t).
        const int host_flags = ConvertMessageFlags(flags);
        if (host_flags < 0) {
                return -1;
        }
        ...
        ssize_t   result   = 0;
        if (addr == nullptr) {
                result = ::recv(socket, static_cast<void*>(buf), static_cast<size_t>(host_len),
                                host_flags);
        } else {
                sockaddr_storage host_addr {};
                socklen_t        host_addrlen = sizeof(host_addr);
                result = ::recvfrom(socket, static_cast<void*>(buf),
                                    static_cast<size_t>(host_len), host_flags,
                                    reinterpret_cast<sockaddr*>(&host_addr), &host_addrlen);
                if (result >= 0 &&
                    ConvertHostSockaddr(&host_addr, static_cast<int>(host_addrlen), addr,
                                        addrlen) != 0) {
                        return -1;
                }
        }
...
int KYTY_SYSV_ABI Select(int nfds, void* readfds, void* writefds, void* exceptfds,
        ...
        return result;
#else
        // Round 28: real POSIX select. Same fd remap as Win32, but the first
        // argument must be the highest host fd + 1 on POSIX (ignored on Win32).
        fd_set host_read {};
        fd_set host_write {};
        fd_set host_except {};
        FD_ZERO(&host_read);
        ...
        if (read_ptr == nullptr && write_ptr == nullptr && except_ptr == nullptr) {
                if (host_timeout_ptr != nullptr) {
                        const auto sleep_ms = static_cast<uint64_t>(host_timeout.tv_sec) * 1000 +
                                              static_cast<uint64_t>(host_timeout.tv_usec) / 1000;
                        struct timespec req {};
                        req.tv_sec  = static_cast<time_t>(sleep_ms / 1000);
                        req.tv_nsec = static_cast<long>((sleep_ms % 1000) * 1000000);
        ...
        const int result = ::select(host_nfds, read_ptr, write_ptr, except_ptr, host_timeout_ptr);
        if (result < 0) {
                return SetPosixSocketError();
        }
...
// ---- Round 28: additional real socket backends -------------------------------
...
// Guest msghdr (OrbisNetMsghdr): same field order/widths as POSIX on LP64.
struct GuestMsghdr {
        void*     msg_name       = nullptr;
        uint32_t  msg_namelen    = 0;
        void*     msg_iov        = nullptr;
        int32_t   msg_iovlen     = 0;
...
int64_t KYTY_SYSV_ABI Sendmsg(int s, const void* msg, int flags) {
        PRINT_NAME();
        ...
        const auto* guest = static_cast<const GuestMsghdr*>(msg);
        if (guest->msg_iovlen < 0 || guest->msg_iovlen > 1024) {
                *Posix::GetErrorAddr() = Posix::POSIX_EINVAL;
                return -1;
        }
        // Control data (cmsgs) is not translated: fail closed when present.
        if (guest->msg_control != nullptr && guest->msg_controllen != 0) {
                *Posix::GetErrorAddr() = Posix::POSIX_EOPNOTSUPP;
                return -1;
        }
        ...
        sockaddr_storage host_addr {};
        sockaddr*        host_name    = nullptr;
        socklen_t        host_namelen = 0;
        if (guest->msg_name != nullptr && guest->msg_namelen != 0) {
                int len = 0;
                if (ConvertGuestSockaddr(guest->msg_name, guest->msg_namelen, &host_addr, &len) !=
                    0) {
                        return -1;
                }
                host_name    = reinterpret_cast<sockaddr*>(&host_addr);
                host_namelen = static_cast<socklen_t>(len);
        }

        std::vector<iovec> iov(static_cast<size_t>(guest->msg_iovlen > 0 ? guest->msg_iovlen : 0));
        const auto* guest_iov = static_cast<const GuestIovec*>(guest->msg_iov);
        for (size_t i = 0; i < iov.size(); ++i) {
                iov[i].iov_base = guest_iov[i].iov_base;
                iov[i].iov_len  = static_cast<size_t>(guest_iov[i].iov_len);
        }

        msghdr host_msg {};
        host_msg.msg_name    = host_name;
        host_msg.msg_namelen = host_namelen;
        host_msg.msg_iov     = iov.data();
        host_msg.msg_iovlen  = static_cast<size_t>(iov.size());

        const ssize_t result = ::sendmsg(socket, &host_msg, host_flags);
        if (result < 0) {
                return SetPosixSocketError();
        }
        return static_cast<int64_t>(result);
...
int64_t KYTY_SYSV_ABI Recvmsg(int s, void* msg, int flags) {
        PRINT_NAME();
        ...
        auto* guest = static_cast<GuestMsghdr*>(msg);
        if (guest->msg_iovlen < 0 || guest->msg_iovlen > 1024) {
                *Posix::GetErrorAddr() = Posix::POSIX_EINVAL;
                return -1;
        }
        ...
        sockaddr_storage host_addr {};
        std::vector<iovec> iov(static_cast<size_t>(guest->msg_iovlen > 0 ? guest->msg_iovlen : 0));
        auto* guest_iov = static_cast<const GuestIovec*>(guest->msg_iov);
        for (size_t i = 0; i < iov.size(); ++i) {
                iov[i].iov_base = guest_iov[i].iov_base;
                iov[i].iov_len  = static_cast<size_t>(guest_iov[i].iov_len);
        }
        ...
        msghdr host_msg {};
        host_msg.msg_name    = (guest->msg_name != nullptr ? &host_addr : nullptr);
        host_msg.msg_namelen = (guest->msg_name != nullptr ? sizeof(host_addr) : 0);
        host_msg.msg_iov     = iov.data();
        host_msg.msg_iovlen  = static_cast<size_t>(iov.size());

        const ssize_t result = ::recvmsg(socket, &host_msg, host_flags);
        if (result < 0) {
                return SetPosixSocketError();
        }

        if (guest->msg_name != nullptr && (host_msg.msg_namelen != 0)) {
                uint32_t out_len = guest->msg_namelen;
                if (ConvertHostSockaddr(&host_addr, static_cast<int>(host_msg.msg_namelen),
                                        guest->msg_name, &out_len) == 0) {
                        guest->msg_namelen = out_len;
                }
        }
        guest->msg_flags = 0;
        return static_cast<int64_t>(result);
...
int KYTY_SYSV_ABI SocketAbort(int s) {
        PRINT_NAME();
        ...
        // Abort semantics: kill pending I/O on the socket without closing the
        // descriptor. shutdown(both directions) is the portable approximation.
#if defined(_WIN32)
...
int KYTY_SYSV_ABI Ioctl(int s, uint32_t request, void* arg) {
        PRINT_NAME();
        ...
        constexpr uint32_t ORBIS_FIONBIO   = 0x8004667eu;   // same numeric value as Win32/POSIX
        constexpr uint32_t ORBIS_FIONREAD  = 0x4004667fu;

#if defined(_WIN32)
        if (request == ORBIS_FIONBIO || request == ORBIS_FIONREAD) {
                u_long value = (arg != nullptr ? *static_cast<u_long*>(arg) : 0);
                if (::ioctlsocket(socket, static_cast<long>(request), &value) == SOCKET_ERROR) {
                        return SetPosixSocketError();
                }
                if (request == ORBIS_FIONREAD && arg != nullptr) {
                        *static_cast<u_long*>(arg) = value;
                }
                return 0;
        }
#else
        if (request == ORBIS_FIONBIO) {
                if (arg == nullptr) {
                        *Posix::GetErrorAddr() = Posix::POSIX_EFAULT;
                        return -1;
                }
                ...
                const int new_flags = (*static_cast<const int*>(arg) != 0 ? flags | O_NONBLOCK
                                                                          : flags & ~O_NONBLOCK);
                if (::fcntl(socket, F_SETFL, new_flags) != 0) {
                        return SetPosixSocketError();
                }
...
[[maybe_unused]] static void CopyIpv4String(char* dst, size_t dst_size, const sockaddr* addr) {
#if defined(_WIN32)
...
[[maybe_unused]] static void CopyIpv4Netmask(char* dst, size_t dst_size, uint8_t prefix_len) {
#if defined(_WIN32)
...
namespace LibKernel = ::Libs::LibKernel;
