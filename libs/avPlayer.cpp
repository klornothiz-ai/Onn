...
private:
	...
	void PrepareVideo(AVFrame* src) {
		...
		// Round 33: bounds-check the NV12 copy. A malformed or larger-than-
		// expected frame (src->width > pitch or src->height > h) would
		// overflow the destination buffer. Clamp the copy to the allocated
		// dimensions instead of trusting FFmpeg's dimensions blindly.
		const uint32_t copy_w = static_cast<uint32_t>(std::min<int>(src->width, static_cast<int>(pitch)));
		const uint32_t copy_h = static_cast<uint32_t>(std::min<int>(src->height, static_cast<int>(h)));
		for (int y = 0; y < static_cast<int>(copy_h); y++) {
			std::memcpy(dst + y * pitch, nv12->data[0] + y * nv12->linesize[0], copy_w);
		}
		auto* c = dst + pitch * h;
		for (int y = 0; y < static_cast<int>(copy_h) / 2; y++) {
			std::memcpy(c + y * pitch, nv12->data[1] + y * nv12->linesize[1], copy_w);
		}
