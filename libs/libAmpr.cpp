...
static const char g_module[] = "libAmpr";
#include "libs/libs.h"
...
static int GetHostPathStat(const std::string& host_path, LibKernel::FileSystem::FileStat* st) {
	...
	if (is_dir) {
		...
		stat.st_blocks  = (stat.st_size + 511) / 512;
                ...
                uint64_t at_us = 0;
                uint64_t wt_us = 0;
                Common::File::GetLastAccessAndWriteTimeUTC(host_path, &at_us, &wt_us);
                at = Common::DateTime::FromUnixTime(static_cast<int64_t>(at_us / 1000000));
                wt = Common::DateTime::FromUnixTime(static_cast<int64_t>(wt_us / 1000000));
	}
...
static bool AllocateAmmAutoDirectMemory(uint64_t size, int memory_type, uint64_t* dmem_offset) {
	...
	if (LibKernel::Memory::KernelAllocateDirectMemory(
	        0, LibKernel::Memory::KernelGetDirectMemorySize(), size, AMM_PAGE_SIZE, memory_type,
                reinterpret_cast<uint64_t*>(&allocated)) != OK) {
		return false;
	}
...
static int ExecuteAmmMapCommand(const CommandBufferState::AmmMapCommand& command) {
	...
	void* addr = reinterpret_cast<void*>(command.va);
        ...
        return LibKernel::Memory::KernelMapDirectMemory2(
            &addr, command.size, command.type, NormalizeAmmProtection(command.prot),
            static_cast<uint64_t>(dmem_offset), &mapped);
}
		...
		switch (entry.kind) {
			...
			case CommandKind::KernelEvent: {
				...
				auto        result  = LibKernel::EventQueue::KernelTriggerUserEvent(
                                    eq, command.id, static_cast<uint32_t>(command.data));
				if (result != OK) {
			...
			case CommandKind::AmmMap: {
				...
				if (command.kind == AmmCommandKind::Unmap) {
                                        result = LibKernel::Memory::KernelMunmap(
                                            reinterpret_cast<void*>(command.va), command.size);
				} else {
...
static int KYTY_SYSV_ABI AmmGiveDirectMemory(int64_t search_start, int64_t search_end,
	...
	int64_t allocated = 0;
        int     result = LibKernel::Memory::KernelAllocateDirectMemory(
            search_start, search_end, size, align, 0, reinterpret_cast<uint64_t*>(&allocated));
	if (result != OK) {
		return result;
	}
