...
static const char g_module[] = "libDbgAsan";
...
using Common::Platform::sys_dbg_stack_info_t;
using Common::Platform::SysStackUsage;
...
static int KernelFree(void* addr, size_t size) {
        FreeShadow(reinterpret_cast<uintptr_t>(addr),
                   reinterpret_cast<uintptr_t>(addr) + size - 1);
        return 0;
}
...
static void KYTY_SYSV_ABI asan_init() {
        ...
        LibKernel::Memory::RegisterCallbacks(KernelAlloc, KernelFree, nullptr, nullptr);
}
