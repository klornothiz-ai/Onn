...
// ---- Round 28: the socket data path (connect/send/recv/...) ---------------
...
int64_t KYTY_SYSV_ABI NetSend(int s, const void* buf, uint64_t len, int flags) {
        ...
        if (result < 0) {
                *GetNetErrorAddr() = PosixToNetError(*Posix::GetErrorAddr());
                return static_cast<int64_t>(*GetNetErrorAddr());
        }
        return result;
}

int64_t KYTY_SYSV_ABI NetSendto(int s, const void* buf, uint64_t len, int flags,
                                const void* addr, uint32_t addrlen) {
        const auto result = Net::Sendto(s, buf, len, flags, addr, addrlen);
        if (result < 0) {
                *GetNetErrorAddr() = PosixToNetError(*Posix::GetErrorAddr());
                return static_cast<int64_t>(*GetNetErrorAddr());
        }
...
int64_t KYTY_SYSV_ABI NetSendmsg(int s, const void* msg, int flags) {
        const auto result = Net::Sendmsg(s, msg, flags);
        if (result < 0) {
                *GetNetErrorAddr() = PosixToNetError(*Posix::GetErrorAddr());
                return static_cast<int64_t>(*GetNetErrorAddr());
        }
...
int64_t KYTY_SYSV_ABI NetRecvfrom(int s, void* buf, uint64_t len, int flags, void* addr,
                                  uint32_t* addrlen) {
        const auto result = Net::Recvfrom(s, buf, len, flags, addr, addrlen);
        if (result < 0) {
                *GetNetErrorAddr() = PosixToNetError(*Posix::GetErrorAddr());
                return static_cast<int64_t>(*GetNetErrorAddr());
        }
...
int64_t KYTY_SYSV_ABI NetRecvmsg(int s, void* msg, int flags) {
        const auto result = Net::Recvmsg(s, msg, flags);
        if (result < 0) {
                *GetNetErrorAddr() = PosixToNetError(*Posix::GetErrorAddr());
                return static_cast<int64_t>(*GetNetErrorAddr());
        }
...
int KYTY_SYSV_ABI NetSocketAbort(int s) {
        return NET_CALL(Net::SocketAbort(s));
}

int KYTY_SYSV_ABI NetIoctl(int s, uint32_t request, void* arg) {
        return FinishSocketCall(Net::Ioctl(s, request, arg));
}

static int KYTY_SYSV_ABI NetTerm() {
        return NET_CALL(Net::NetTerm());
}

uint64_t KYTY_SYSV_ABI NetHtonll(uint64_t host64) {
        return ((host64 & 0x00000000000000ffull) << 56u) |
               ((host64 & 0x000000000000ff00ull) << 40u) |
               ((host64 & 0x0000000000ff0000ull) << 24u) |
               ((host64 & 0x00000000ff000000ull) << 8u) |
               ((host64 & 0x000000ff00000000ull) >> 8u) |
               ((host64 & 0x0000ff0000000000ull) >> 24u) |
               ((host64 & 0x00ff000000000000ull) >> 40u) |
               ((host64 & 0xff00000000000000ull) >> 56u);
}

uint64_t KYTY_SYSV_ABI NetNtohll(uint64_t net64) {
        return NetHtonll(net64);
}

int KYTY_SYSV_ABI NetEtherStrton(const char* str, Net::NetEtherAddr* n) {
        return NET_CALL(Net::NetEtherStrton(str, n));
}
...
LIB_DEFINE(InitNet_1_Net) {
        ...
        // ---- Round 28: the socket data path (NIDs verified against the
        // public shadPS4 libSceNet registration table) ----
        LIB_FUNC("OXXX4mUk3uk", LibNet::NetConnect);          // sceNetConnect
        LIB_FUNC("beRjXBn-z+o", LibNet::NetSend);             // sceNetSend
        LIB_FUNC("gvD1greCu0A", LibNet::NetSendto);           // sceNetSendto
        LIB_FUNC("2eKbgcboJso", LibNet::NetSendmsg);          // sceNetSendmsg
        LIB_FUNC("9wO9XrMsNhc", LibNet::NetRecv);             // sceNetRecv
        LIB_FUNC("304ooNZxWDY", LibNet::NetRecvfrom);         // sceNetRecvfrom
        LIB_FUNC("wvuUDv0jrMI", LibNet::NetRecvmsg);          // sceNetRecvmsg
        LIB_FUNC("xphrZusl78E", LibNet::NetGetsockopt);       // sceNetGetsockopt
        LIB_FUNC("TCkRD0DWNLg", LibNet::NetGetpeername);      // sceNetGetpeername
        LIB_FUNC("zJGf8xjFnQE", LibNet::NetSocketAbort);      // sceNetSocketAbort
        LIB_FUNC("ghqRRVQxqKo", LibNet::NetIoctl);            // sceNetIoctl
        LIB_FUNC("cTGkc6-TBlI", LibNet::NetTerm);             // sceNetTerm
        LIB_FUNC("3CHi1K1wsCQ", LibNet::NetHtonll);           // sceNetHtonll
        LIB_FUNC("tOrRi-v3AOM", LibNet::NetNtohll);           // sceNetNtohll
        LIB_FUNC("b-bFZvNV59I", LibNet::NetEtherStrton);      // sceNetEtherStrton
}
...
static int ParseEmptyUri(SceHttpUriElement* out, void* pool, size_t* require, size_t prepare) {
        ...
        if (out != nullptr) {
                *out = SceHttpUriElement{};   // trivially value-initialised
                out->opaque = 1;
        }
...
static int KYTY_SYSV_ABI Http2CreateTemplate(int lib_http2_ctx_id, const char* user_agent,
        ...
        Http2Template tmpl{};
        tmpl.context_id         = lib_http2_ctx_id;
        tmpl.user_agent         = user_agent != nullptr ? user_agent : "";
        tmpl.http_ver           = http_ver;
        tmpl.is_auto_proxy_conf = is_auto_proxy_conf;
        g_http2_templates[tmpl_id] = std::move(tmpl);
...
static int KYTY_SYSV_ABI Http2CreateRequestWithURL(int tmpl_id, const char* method, const char* url,
        ...
        g_http2_requests[req_id] = std::move(req);
...
static int KYTY_SYSV_ABI RandomGetRandomNumber(void* buf, size_t size) {
        ...
        // Round 29: REAL kernel entropy. The previous implementation was a
        // fixed-seed LCG (deterministic across process runs -- a correctness
        // bug for any game logic that consumes this NID). getrandom(2)
        // blocks until the CRNG is ready, which is exactly the guarantee a
        // caller of "random number" expects.
        if (size != 0) {
                ssize_t got = 0;
                size_t  off = 0;
                auto*   out = static_cast<uint8_t*>(buf);
                while (off < size) {
                        got = ::getrandom(out + off, size - off, 0);
                        if (got < 0) {
                                return RANDOM_ERROR_INVALID;
                        }
