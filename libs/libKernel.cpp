...
struct ExportSymbolRecord {
        std::string name;
        uint64_t    vaddr;
};

const ExportSymbolRecord* kernel_find_export_symbol(const Loader::ModuleInfo::ExportSymbols* symbols,
        ...
        for (size_t i = 0; i < symbols->names.size(); i++) {
                if (symbols->names[i] == name) {
                        static ExportSymbolRecord record;
                        record.name  = name;
                        record.vaddr = i < symbols->values.size() ? symbols->values[i] : 0;
                        return &record;
                }
        }
        return nullptr;
...
static int KYTY_SYSV_ABI KernelStopUnloadModule(KernelModule handle, size_t args, const void* argp,
	...
	if (g_get_thread_atexit_count_func != nullptr &&
        ...
        int result = rt->StopModule(program, args, argp, 0);
...
static int KYTY_SYSV_ABI KernelFsync(int fd) {
	...
	// Round 29: real flush-to-disk through the host descriptor (sockets are
	// not fsync-able -- return EINVAL like FreeBSD does).
	if (Network::Net::IsSocket(fd)) {
		return 22;   // EINVAL
	}
	return POSIX_CALL(fsync(fd));
}
...
static void KYTY_SYSV_ABI KernelRtldSetApplicationHeapAPI(void* api[]) {
	...
	auto* rt = Common::Singleton<Loader::RuntimeLinker>::Instance();
        rt->SetApplicationHeapApi(reinterpret_cast<void* (*)(size_t)>(api[0]),
                                  reinterpret_cast<void (*)(void*)>(api[1]),
                                  reinterpret_cast<void* (*)(size_t, size_t)>(api[6]));
...
static int KYTY_SYSV_ABI open(const char* path, int flags, int mode) {
        int32_t fd   = -1;
        int     rc   = FileSystem::KernelOpen(path, flags, static_cast<uint16_t>(mode), &fd);
        return rc == OK ? static_cast<int>(fd) : POSIX_N_CALL(FileSystem::KernelOpen(path, flags, static_cast<uint16_t>(mode), &fd));
}
...
int KYTY_SYSV_ABI KernelSyncOnAddressWait(volatile uint32_t* address, uint32_t expected,
        ...
        const int64_t timeout = timeout_micros != nullptr ? static_cast<int64_t>(*timeout_micros) : -1;
        return LibKernel::SyncOnAddress::Wait32(address, expected, timeout,
...
int KYTY_SYSV_ABI KernelSyncOnAddressWait64(volatile uint64_t* address, uint64_t expected,
        ...
        const int64_t timeout = timeout_micros != nullptr ? static_cast<int64_t>(*timeout_micros) : -1;
        return LibKernel::SyncOnAddress::Wait64(address, expected, timeout,
	                                        LibKernel::KernelDispatchPendingSignalForCurrentThread);
}
...
LIB_DEFINE(InitLibKernel_1_Posix) {
	...
	LIB_FUNC("2Z+PpY6CaJg", Posix::pthread_mutex_unlock);
        LIB_FUNC("ttHNfU+qDBU",
                 static_cast<int (*)(void**, const void**)>(&Posix::pthread_mutex_init));
...
LIB_DEFINE(InitLibKernel_1_Pthread) {
	...
	LIB_FUNC("2Of0f+3mhhE", LibKernel::PthreadMutexDestroy);
        LIB_FUNC("cmo1RIYva9o",
                 static_cast<int (*)(LibKernel::PthreadMutex*, const LibKernel::PthreadMutexattr*)>(
                     &LibKernel::PthreadMutexInit));
