...
static const char g_module[] = "libSaveData";
#include "libs/libs.h"
...
// ---- Round 28: real file-backed persistence ---------------------------------
//
// PARAM.bin: a small documented container written INSIDE the mounted save
// directory. Layout: magic "PLSD" (u32) | version (u32, =1) | payload size
// (u32) | raw SaveDataParam bytes. It is NOT a Sony PARAM.SFO -- games never
// parse this file themselves, they go through sceSaveDataSetParam/GetParam,
// and the emulator persists whatever round-trips those two calls.
// icon0.png: the raw bytes the guest hands to sceSaveDataSaveIcon.

static constexpr uint32_t SAVE_PARAM_MAGIC   = 0x44534C50u;   // "PLSD"
static constexpr uint32_t SAVE_PARAM_VERSION = 1u;

static std::string mounted_file_path(const SaveDataMountPoint* mount_point,
                                      const char* file_name) {
        if (mount_point == nullptr) {
                return {};
        }
        return LibKernel::FileSystem::GetRealFilename(std::string(mount_point->data) + "/" +
                                                      file_name);
}

static bool write_save_param(const std::string& path, const SaveDataParam* param) {
        if (path.empty() || param == nullptr) {
                return false;
        }
        Common::File::File file;
        if (!file.Open(path, Common::File::Mode::Write)) {
                return false;
        }
        const uint32_t header[3] = {SAVE_PARAM_MAGIC, SAVE_PARAM_VERSION,
                                    static_cast<uint32_t>(sizeof(SaveDataParam))};
        if (!file.Write(header, sizeof(header)) || !file.Write(param, sizeof(SaveDataParam)) ||
            !file.Flush()) {
                return false;
        }
        return true;
...
static bool read_save_param(const std::string& path, SaveDataParam* param) {
        if (path.empty() || param == nullptr) {
                return false;
        }
        Common::File::File file;
        ...
        uint32_t header[3] = {};
        if (!file.Read(header, sizeof(header)) || header[0] != SAVE_PARAM_MAGIC ||
            header[1] != SAVE_PARAM_VERSION || header[2] != sizeof(SaveDataParam)) {
                return false;
        }
        return file.Read(param, sizeof(SaveDataParam));
}
...
int KYTY_SYSV_ABI SaveDataDirNameSearch(const SaveDataDirNameSearchCond* cond,
        ...
        if (Common::File::IsDirectoryExisting(root)) {
                for (const auto& entry: Common::File::GetDirEntriesEx(root)) {
...
int KYTY_SYSV_ABI SaveDataGetParam(const SaveDataMountPoint* mount_point, uint32_t param_type,
        ...
        if (param_type == 0) {
                ...
                // Round 28: read the REAL persisted param from the mounted save
                // directory. A save with no PARAM.bin yet reads back as zeroed (the
                // same observable state as a freshly created save on hardware).
                auto* out      = static_cast<SaveDataParam*>(param_buf);
                *out           = {};
                const bool ok = read_save_param(mounted_file_path(mount_point, "PARAM.bin"), out);
                LOGF("\t PARAM.bin read = %s\n", ok ? "yes" : "no (zeroed)");
                if (got_size != nullptr) {
...
int KYTY_SYSV_ABI SaveDataLoadIcon(const SaveDataMountPoint* mount_point, SaveDataIcon* icon) {
        ...
        // Round 28: read the REAL icon file persisted by sceSaveDataSaveIcon.
        icon->data_size = 0;
        Common::File::File file;
        if (!file.Open(mounted_file_path(mount_point, "icon0.png"), Common::File::Mode::Read)) {
                return OK;               // no icon stored: data_size stays 0
        }
        const int64_t size = file.Size();
        if (size <= 0 || icon->buf == nullptr) {
                return OK;
        }
        const auto to_read = static_cast<size_t>(size <= static_cast<int64_t>(icon->buf_size)
                                                     ? size
                                                     : static_cast<int64_t>(icon->buf_size));
        if (file.Read(icon->buf, to_read)) {
                icon->data_size = static_cast<uint64_t>(to_read);
        }
...
int KYTY_SYSV_ABI SaveDataSetParam(const SaveDataMountPoint* mount_point, uint32_t param_type,
        ...
        if (param_type == 0) {
                ...
                // Round 28: persist to the mounted save directory (fail-closed with
                // an honest error when the write is impossible).
                if (!write_save_param(mounted_file_path(mount_point, "PARAM.bin"), p)) {
                        return SAVE_DATA_ERROR_PARAMETER;
                }
        } else {
...
int KYTY_SYSV_ABI SaveDataSaveIcon(const SaveDataMountPoint* mount_point,
        ...
        // Round 28: persist the raw icon bytes into the mounted save directory.
        if (icon->buf == nullptr || icon->data_size == 0 || icon->data_size > icon->buf_size) {
                return SAVE_DATA_ERROR_PARAMETER;
        }
        Common::File::File file;
        if (!file.Open(mounted_file_path(mount_point, "icon0.png"), Common::File::Mode::Write)) {
                return SAVE_DATA_ERROR_PARAMETER;
        }
        if (!file.Write(icon->buf, static_cast<size_t>(icon->data_size)) || !file.Flush()) {
                return SAVE_DATA_ERROR_PARAMETER;
        }
