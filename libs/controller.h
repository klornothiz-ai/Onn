...
// Idempotently create the controller state machine (host-input pad connected).
// The KYTY_SUBSYSTEM_INIT hook is `inline` and was never linked/called, leaving
// g_controller null; this gives the emulator and tests a real entry point to
// bring the input subsystem up. Safe to call more than once.
void ControllerEnsureInitialized();
