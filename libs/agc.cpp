...
using ::Graphics::RenderContext;
using ::Graphics::Gpu;
using ::Graphics::RenderDocInit;
using ::Graphics::WindowInit;
using ::Graphics::WindowShutdown;
...
void GraphicsDbgDumpDcb(const char* type, uint32_t num_dw, uint32_t* cmd_buffer) {
	...
	if (Config::CommandBufferDumpEnabled() && num_dw > 0 && cmd_buffer != nullptr && g_renderer != nullptr) {
                Common::File::File f;
                auto         file_name = std::filesystem::path(Config::GetCommandBufferDumpFolder()) /
		                         [&] { char buffer[128]; std::snprintf(buffer, sizeof(buffer), "%04u_%04u_buffer_%s.log", static_cast<unsigned>(g_renderer->GetGpu().GetFrameNum()), static_cast<unsigned>(id++), type); return std::string(buffer); }();
                Common::File::CreateDirectories(file_name.parent_path().string());
                f.Create(file_name.string());
		if (f.IsInvalid()) {
			...
			return;
                ...
                auto* fp = f.GetHandle();
                Pm4::DumpPm4PacketStream(fp, cmd_buffer, 0, num_dw);
		f.Close();
	}
}
...
using ::Pm4::ContextStateOperation;
using ::Pm4::KYTY_PM4_LEN;
using ::Pm4::KYTY_PM4_R;
using ::Pm4::KYTY_PM4_IT;
using ::Pm4::KYTY_PMN;
using ::Pm4::KYTY_PMN_LEN;
using ::Pm4::PmN::KYTY_PMN_R;
...
struct CommandBuffer {
	...
	void DbgDump() const {
		if (!Config::GraphicsDebugDumpEnabled() ||
                    static_cast<Config::OutputDirection>(Config::GetPrintfDirection()) ==
	...
	if (h->user_data != nullptr) {
		...
		for (int imm = 0; imm < 4; imm++) {
			...
			for (int i = 0; i < static_cast<int>(h->user_data->sharp_resource_count[imm]); i++) {
                                const auto* sharp = reinterpret_cast<const ShaderSharpResource*>(
                                    h->user_data->sharp_resource_offset[imm]);
                                LOGF("\t\t\t offset_dw[%d] = %04" PRIx16 ", size = %" PRIu16 "\n", i,
                                     static_cast<uint16_t>(sharp[i].offset_dw),
                                     static_cast<uint16_t>(sharp[i].size));
			}
		}
...
int KYTY_SYSV_ABI GraphicsCreateShader(Shader** dst, void* header, const volatile void* code) {
	...
	auto update_addr = [](auto& m) {
        ...
        auto update_addr_array = [](auto& m) {
                const uintptr_t base = reinterpret_cast<uintptr_t>(&m);
                for (auto& e : m) {
                        e = static_cast<typename std::remove_reference<decltype(e)>::type>(
        ...
        if (h->user_data != nullptr) {
                update_addr_array(h->user_data->direct_resource_offset);
                // sharp_resource_offset is an array of guest VA u64 values.
                for (auto& off : h->user_data->sharp_resource_offset) {
                        if (off != 0) {
                                off += reinterpret_cast<uintptr_t>(&off);
                        }
                }
	}
